...
def mega_admin_dashboard():
    ...
    with admin_tabs[1]:
        sec_header("USERS", "Manage Platform Access", "Add/Remove Admins & Users")
        users = get_all_users()
        if users:
            df_users = pd.DataFrame(users, columns=["ID", "Email", "Is Admin", "Created At", "Last Login"])
            df_users["Is Admin"] = df_users["Is Admin"].map({1: "Yes", 0: "No"})
            st.dataframe(df_users, use_container_width=True)

            col_a, col_b = st.columns(2)
            with col_a:
                st.markdown("**Grant Admin Rights**")
                admin_candidates = df_users[df_users["Is Admin"] == "No"]["Email"].tolist()
                admin_email = st.selectbox("Select User", admin_candidates, key="admin_sel")
                if st.button("Make Admin"):
                    if admin_email:
                        success, msg = add_admin_by_email(admin_email)
                        if success:
                            st.success(msg)
                            st.rerun()
                        else:
            ...
            with col_b:
                ...
                del_id = st.selectbox("Select User ID to Delete", df_users["ID"].tolist(), key="del_sel")
    ...
    # ---- TAB 2: ACTIVITY LOGS ----
    with admin_tabs[2]:
        sec_header("LOGS", "System Audit Trail", "Recent system events")
    ...
    with admin_tabs[3]:
        sec_header("SETTINGS", "Configuration Limits", "Platform constraints")
        st.info(f"Maximum File Upload Size: {MAX_FILE_SIZE_MB} MB")
        st.warning("Advanced server configuration requires a physical restart of the Streamlit container.")

    # ---- TAB 4: PLATFORM ANALYTICS ----
    with admin_tabs[4]:
        sec_header("ANALYTICS", "Usage Breakdown", "User interactions")
        if sys_logs:
            df_sys = pd.DataFrame(sys_logs, columns=["User", "Action", "Details", "Timestamp"])
            action_counts = df_sys["Action"].value_counts().reset_index()
            action_counts.columns = ["Action", "Count"]
            fig_pie = px.pie(action_counts, names="Action", values="Count", 
                             title="System Actions Breakdown", hole=0.4,
                             color_discrete_sequence=px.colors.qualitative.Pastel)
            st.plotly_chart(fig_pie, use_container_width=True)
        else:
            st.info("Insufficient data for platform analytics.")
...
def main():
    ...
    # Handle Sidebar Authentication
    login_section()

    # Route to Admin Dashboard if applicable
    if st.session_state.get("logged_in") and st.session_state.get("is_admin"):
        if st.sidebar.checkbox("🛡️ Open Admin Dashboard"):
    ...
    <div style="text-align: center; margin-bottom: 2rem;">
        <h1 style="font-size: 3rem; margin-bottom: 0;">🚀 NEXUS Analytics Pro</h1>
        <p style="color: #64748b; font-size: 1.2rem;">Enterprise-grade Machine Learning & Data Intelligence</p>
    </div>
    """, unsafe_allow_html=True)

    # Data Loading
    data_source = st.radio("Choose Data Source", ["Use Built-in Dataset", "Upload CSV"], horizontal=True)
    
    df = None
    if data_source == "Upload CSV":
        uploaded_file = st.file_uploader(f"Upload CSV (Max {MAX_FILE_SIZE_MB}MB)", type=["csv"])
        if uploaded_file:
            try:
                df, enc = read_csv_with_encoding(uploaded_file)
                st.success(f"File loaded successfully (Encoding: {enc})")
            except Exception as e:
                st.error(f"Error loading file: {e}")
    else:
        df = load_builtin_dataset()
        st.info("Loaded built-in global retail dataset.")

    if df is not None:
        roles = detect_column_types(df)
        df_clean = smart_clean(df, roles)
        ...
        # --- Data Overview ---
        with app_tabs[0]:
            sec_header("DATA", "Exploratory Overview")
            st.dataframe(df_clean.head(100), use_container_width=True)
            
            c1, c2, c3 = st.columns(3)
            with c1:
                st.metric("Total Rows", fmt_num(len(df_clean)))
            with c2:
                st.metric("Total Columns", len(df_clean.columns))
            with c3:
                missing = df.isnull().sum().sum()
                st.metric("Missing Values Cleaned", fmt_num(missing))

        # --- Predictive ML ---
        with app_tabs[1]:
            sec_header("ML", "Ensemble Regression", "Predict numerical outcomes")
            if not roles["numeric"]:
                st.warning("No numeric columns available for prediction.")
            else:
                target_col = st.selectbox("Select Target Variable", roles["numeric"])
                feature_cols = st.multiselect("Select Features", [c for c in df_clean.columns if c != target_col], default=[c for c in roles["numeric"] if c != target_col][:3])
                
                if st.button("Train Ensemble Model", type="primary") and feature_cols:
                    with st.spinner("Training Random Forest, Gradient Boosting, Ridge, and ElasticNet..."):
                        df_json = df_clean.to_json()
                        ensemble, le_map, scaler, cv_r2, cv_mape, imp, perm_imp, final_cols = train_ml_ensemble(df_json, target_col, feature_cols)
                        
                        st.success("Model trained successfully!")
                        c1, c2 = st.columns(2)
                        c1.metric("Cross-Validated R²", f"{cv_r2:.3f}" if cv_r2 else "N/A")
                        c2.metric("MAPE (Error)", f"{cv_mape:.2f}%" if cv_mape else "N/A")
        ...
        # --- Forecasting ---
        with app_tabs[2]:
            sec_header("TIME SERIES", "Advanced Forecasting", "Prophet / Exponential Smoothing")
            if not roles["date"]:
                st.warning("No date columns detected for forecasting.")
            else:
                date_col = st.selectbox("Select Date Column", roles["date"])
                val_col = st.selectbox("Select Value to Forecast", roles["numeric"], key="fc_val")
                horizon = st.slider("Forecast Horizon", 10, 365, 90)
                freq = st.selectbox("Frequency", ["ME", "W"], format_func=lambda x: "Monthly" if x == "ME" else "Weekly")
                
                if st.button("Generate Forecast", type="primary"):
                    with st.spinner("Building forecast model..."):
                        date_json = df_clean[date_col].to_json()
                        val_json = df_clean[val_col].to_json()
                        hist, fcast, algo = build_forecast(date_json, val_json, horizon, freq)
                        
                        if hist is not None:
                            st.success(f"Forecast generated using: {algo}")
                            fig = go.Figure()
                            fig.add_trace(go.Scatter(x=hist['Date'], y=hist['Value'], name="Historical", line=dict(color="#4f46e5")))
                            fig.add_trace(go.Scatter(x=fcast['Date'], y=fcast['Value'], name="Forecast", line=dict(color="#06b6d4", dash="dash")))
                            fig.add_trace(go.Scatter(x=fcast['Date'], y=fcast['Upper'], fill=None, mode='lines', line_color='rgba(0,0,0,0)', showlegend=False))
                            fig.add_trace(go.Scatter(x=fcast['Date'], y=fcast['Lower'], fill='tonexty', mode='lines', line_color='rgba(0,0,0,0)', name="Confidence Interval", fillcolor='rgba(6, 182, 212, 0.2)'))
                            st.plotly_chart(fig, use_container_width=True)
                        else:
                            st.error("Not enough data or missing dependencies to build forecast.")

        # --- Clustering & Anomalies ---
        with app_tabs[3]:
            sec_header("UNSUPERVISED", "Pattern Discovery", "Clustering & Outliers")
            clust_features = st.multiselect("Clustering Features", roles["numeric"], default=roles["numeric"][:2], key="clust_feat")
            
            if st.button("Run Analysis", type="primary") and len(clust_features) >= 2:
                with st.spinner("Processing..."):
                    df_json = df_clean.to_json()
                    
                    # Anomalies
                    anomalies, iforest = detect_anomalies_iforest(df_json, clust_features)
                    st.error(f"Detected {sum(anomalies)} anomalies using Isolation Forest.")
                    
                    # Clustering
                    labels, sil, coords, inertias, var_ratio, model = run_advanced_clustering(df_json, clust_features)
                    if coords is not None:
                        df_plot = pd.DataFrame(coords, columns=["PCA1", "PCA2"])
                        df_plot["Cluster"] = labels.astype(str)
                        df_plot["Anomaly"] = anomalies
                        
                        fig = px.scatter(df_plot, x="PCA1", y="PCA2", color="Cluster", symbol="Anomaly", 
                                         title="2D PCA Cluster Visualization (Crosses = Anomalies)",
                                         color_discrete_sequence=px.colors.qualitative.Set1)
                        st.plotly_chart(fig, use_container_width=True)
                        if sil:
                            st.info(f"Silhouette Score (Cluster Quality): {sil:.3f}")

        # --- Customer Intelligence ---
        with app_tabs[4]:
            sec_header("BUSINESS", "RFM & Market Basket", "Customer segmentation")
            st.markdown("### RFM Segmentation")
            id_col = st.selectbox("Customer ID Column", ["—"] + roles["id"] + roles["categorical"])
            sales_col = st.selectbox("Monetary/Sales Column", roles["numeric"], key="rfm_sales")
            rfm_date = st.selectbox("Date Column", roles["date"], key="rfm_date")
            
            if st.button("Compute RFM", type="primary"):
                rfm = compute_rfm(df_clean, rfm_date, sales_col, id_col)
                if rfm is not None:
                    st.dataframe(rfm.head())
                    fig = px.pie(rfm, names="Segment", title="Customer Segments Distribution", hole=0.3)
                    st.plotly_chart(fig, use_container_width=True)
                else:
                    st.warning("Could not compute RFM. Ensure you selected a valid ID column.")
